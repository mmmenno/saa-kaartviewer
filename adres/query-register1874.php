...
SELECT ?aladr ?birth ?litname ?adrstr ?deed ?po 
  ...
  BIND (CONCAT(?streetname,\' \',?huisnr) AS ?adrstr) .
...
if(isset($data['results']['bindings'])){
	foreach ($data['results']['bindings'] as $key => $rec) {
		...
		$registratie['label'] = "Register 1974-93: " . $rec['litname']['value'];

		if(isset($rec['birth']) && isset($rec['adrstr'])){
			$registratie['persondescription'] = "Geboren op " . dutchdate($rec['birth']['value']) . ", wonende " . $rec['adrstr']['value'];
		}elseif(isset($rec['adrstr'])){
			$registratie['persondescription'] = "Wonende " . $rec['adrstr']['value'];
		}
